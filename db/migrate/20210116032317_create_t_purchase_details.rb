class CreateTPurchaseDetails < ActiveRecord::Migration[6.0]
  def change
    ...
    create_table :t_purchase_details do |t|
      ...
      t.references :purchase_id, type: :integer, null: false, foreign_key: {to_table: :t_purchases, primary_key: :id} #入荷ID
      t.integer :line_number, null: false #行番号
      t.integer :sort_number, null: false #ソート番号
      t.references :send_order_details_id, type: :integer, null: false, foreign_key: {to_table: :t_send_order_details, primary_key: :id} #発注明細ID
      t.references :product_id, type: :string, limit: 8, null: false, foreign_key: {to_table: :m_products, primary_key: :id} #商品ID
